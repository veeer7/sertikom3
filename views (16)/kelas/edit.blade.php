        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 sm:p-8">
            <form action="{{ route('kelas.update', $kelas->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <x-text-input name="level_kelas" placeholder="Contoh: X, XI, XII" class="mt-2 w-full" :value="old('level_kelas', $kelas->level_kelas)" />
                    <x-input-error :messages="$errors->get('level_kelas')" class="mt-2" />
                </div>

                <div>
                    <x-input-label value="Nama Kelas" />
                    <x-text-input name="nama_kelas" placeholder="Masukkan nama kelas" class="mt-2 w-full" :value="old('nama_kelas', $kelas->nama_kelas)" />
