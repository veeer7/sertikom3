        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 sm:p-8">
            <form action="{{ route('tahun-ajar.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <x-text-input name="kode_tahun_ajar" placeholder="Contoh: 2024/2025" class="mt-2 w-full" :value="old('kode_tahun_ajar')" />
                    <x-input-error :messages="$errors->get('kode_tahun_ajar')" class="mt-2" />
                </div>

                <div>
                    <x-input-label value="Nama Tahun Ajar" />
                    <x-text-input name="nama_tahun_ajar" placeholder="Masukkan nama tahun ajar" class="mt-2 w-full" :value="old('nama_tahun_ajar')" />
